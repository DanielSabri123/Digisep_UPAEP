                    ...
                    success: function (data, textStatus, jqXHR) {
                        ...
                        completeProgress.then(resp => {
                            ...
                            if (bandera === "success") {
                                ...
                                swal("¡Proceso completado!", "<p>Las alumnos han sido descargados correctamente. Se descargaron: <b>" + contador + "</b> calificaciones</p><small><strong>Consulta la carrera que desees.</strong></small>", "success").then(resp => {
                                ...
                                if (bandera === "error") {
                                    ...
                                    var id = data.split("¬")[1].toString().trim();
                                    if (bandera === "sinCarrera") {
                                        var matr = data.split("¬")[2].toString().trim();
                                        swal('¡Upps!',
                                                'El id de la carrera asignada al alumno  con matrícula <b>"' + matr +
                                                "</b> no está registrada en el sistema, por favor rectifique la clave o registre la carrera para continuar. \n" +
                                                "Los registros posteriores a este alumno no se realizaron", 'error');
                                    } else if (bandera === 'sinAlumno') {
                                        var matr = data.split("¬")[2].toString().trim();
                                        swal('¡Upps!',
                                                'No se encontró dentro del sistema al alumno  con matrícula <b>"' + matr +
                                                "\nLos registros posteriores a este alumno no se realizaron.", 'error');
                                        $("#progress-loader").hide();
                                    } else if (bandera === 'sinMateria') {
                                        var cve = data.split("¬")[2].toString().trim();
                                        swal('¡Upps!',
                                                'No se encontró dentro del sistema la matería con clave <b>' + cve +
                                                "</b> para el alumno seleccionado.\nLos registros posteriores a este alumno no se realizaron", 'error');
                                        $("#progress-loader").hide();
                                    } else if (bandera === 'certificadoActivo') {
                                        var matr = data.split("¬")[2].toString().trim();
                                        swal('¡Upps!', 'El alumno  con matrícula ' + matr + ' cuenta con certificados registrados, no es posible actualizar sus calificaciones', 'warning');
                                    } else {
